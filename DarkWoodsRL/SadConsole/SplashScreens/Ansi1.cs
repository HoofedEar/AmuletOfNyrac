...
public class Ansi1 : ScreenSurface
{
    ...
    public Ansi1(IFont font, IFont.Sizes fontSize) : base(GameHost.Instance.ScreenCellsX, GameHost.Instance.ScreenCellsY)
    {
        ...
        // Create surface
        _ansiChild = new ScreenSurface(80, 27);
        ...
        // Read the ansi file
        System.Reflection.Assembly assembly = typeof(DarkWoodsRL.PlayerActionHelper).Assembly;
        using Stream stream = assembly.GetManifestResourceStream("DarkWoodsRL.Resources.logo.ans")!;
        using BinaryReader sr = new BinaryReader(stream);

        SadConsole.Ansi.Document document = Ansi.Document.FromBytes(sr.ReadBytes((int)stream.Length));
        SadConsole.Private.Ansi.AnsiWriter writer = new Private.Ansi.AnsiWriter(document, _ansiChild.Surface);
        ...
        var drawMadeWith = new Instructions.DrawString(madeWithString) { TotalTimeToPrint = (float)TimeSpan.FromSeconds(0.4d).TotalSeconds };
        ...
        var drawSadConsole = new Instructions.DrawString(sadConsoleString) { TotalTimeToPrint = (float)TimeSpan.FromSeconds(0.3d).TotalSeconds };
