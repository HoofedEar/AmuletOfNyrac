...
internal class PlayerFOVController : RogueLikeComponentBase<RogueLikeEntity>
{
    ...
    private void OnAdded(object? s, EventArgs e) => Parent!.PositionChanged += OnMoved;

    private void OnRemoved(object? s, ParentAwareComponentRemovedEventArgs<RogueLikeEntity> e)
        => e.OldParent.PositionChanged -= OnMoved;

    private void OnMoved(object? sender, ValueChangedEventArgs<Point> valueChangedEventArgs)
        => CalculateFOV();
}